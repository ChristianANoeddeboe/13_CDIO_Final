...
public class MySQLProduktBatchDAO implements ProduktBatchDAO{
	...
	public ProduktBatchDTO getProduktBatch(int pbId) throws DALException {
		ResultSet rs = Connector.doQuery("SELECT * FROM produktbatchview WHERE pb_id = " + pbId); // View
		try {
	...
	public List<ProduktBatchDTO> getProduktBatchList() throws DALException {
		...
		ResultSet rs = Connector.doQuery("SELECT * FROM produktbatchview");
		try
		{
			while (rs.next()) 
			{
	...
	public void deleteProduktBatch(int pbID) throws DALException {
		if(Connector.doUpdate("call deleteProductBatch("+pbID+")")==0) {
			throw new DALException("No rows updated in \"Produkt batch\".");
		}
